...
class DigiErpAiConfigSettings(models.TransientModel):
    ...
    digi_erp_ai_ocr_primary = fields.Selection(
        selection=[
            # Kept SHORT on purpose: the selection widget truncates with an
            # ellipsis around 30 characters, and the long version read
            # "AI vision model — nothing to i…" on screen. The detail lives in
            # the help text next to the field, where there is room for it.
            ("llm_vision", "AI vision model"),
            ("paddleocr", "PaddleOCR"),
            ("surya", "Surya"),
            ("tesseract", "Tesseract"),
        ],
