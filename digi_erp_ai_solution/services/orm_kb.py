...
# Paths RELATIVE to the Odoo package root (`odoo/`), as laid out in 18.0.
# Ordered for log readability; the scraper handles each independently.
#
# Odoo 19 split this material into a dedicated `odoo/orm/` package
# (models.py, commands.py, domains.py, decorators.py, environments.py,
# fields*.py). On 18.0 the same classes live in four flat modules at the
# package root, so both the file list and the directory below differ from
# the 19 build of this add-on:
#
#     19 → odoo/orm/models.py, orm/commands.py, orm/domains.py, …
#     18 → odoo/models.py, odoo/fields.py (Field *and* Command),
#          odoo/api.py (decorators *and* Environment),
#          odoo/osv/expression.py (domain operators)
SOURCE_FILES = (
    "models.py",            # BaseModel + create/write/search/read/copy/etc.
    "fields.py",            # Field base class, every concrete field, and the
                            # Command enum (0,1,2,3,4,5,6) for O2M/M2M — which
                            # 19 moved out to its own orm/commands.py.
    "api.py",               # @api.model / @api.depends / @api.onchange / … and
                            # Environment (sudo / with_user / context), split in
                            # 19 into orm/decorators.py + orm/environments.py.
    os.path.join("osv", "expression.py"),  # Domain operators, AND/OR/NOT
                            # combinators — orm/domains.py in 19.
)
...
def _orm_dir(env):
    ...
    Precedence:
      ...
      2. The location of the `odoo` package itself.

    On 18.0 the ORM is not a subpackage: `odoo.models`, `odoo.fields` and
    `odoo.api` sit directly in the `odoo` package root, so that root IS the
    directory to scrape. `odoo.__file__` still points at a real
    odoo/__init__.py at this version (it only becomes None in 19, where
    `odoo` turned into a namespace package), but we read `__path__` first
    anyway so an editable/namespace install resolves too.
    ...
    try:
        ...
        for root in list(getattr(odoo, "__path__", []) or []):
            if os.path.isdir(root) and os.path.exists(
                ...
                return root
        f = getattr(odoo, "__file__", None)
        if f:
            cand = os.path.dirname(f)
...
def _collect_entries(env):
    ...
    if not orm_dir:
        _logger.warning(
            ...
            "ir.config_parameter digi_erp_ai.orm_source_dir to the <odoo> "
            "package root "
