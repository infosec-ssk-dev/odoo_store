...
#
# Le second groupe change l'ENVIRONNEMENT plutôt que les données : `sudo()`
# rendrait le recordset superutilisateur, `with_user()` le ferait basculer sur
# une autre identité. Aujourd'hui le résultat est sérialisé en {model, ids} et
# la qualité « sudo » est perdue avant le retour, donc rien ne s'échappe — mais
# la passerelle promet que « les ACL Odoo gouvernent ce qui est faisable », et
# une méthode qui sert précisément à les contourner n'a rien à faire derrière
# cette promesse. On les refuse explicitement pour que ça reste vrai même si
# _odoo_call venait un jour à chaîner les appels ou à renvoyer le recordset.
DENIED_METHODS = {
    "unlink", "execute", "execute_kw", "exec_workflow",
...
def _humanize_field_label(env, model, fname):
    ...
    Prefers the field's translated `string`; if the field is unknown or has
    no label, turns `amount_total` into "Amount total" so the confirmation
    card never shows a raw technical identifier to a business user.
    """
    try:
...
def build_action_preview(env, params):
    ...
    Retourne un dict destiné au frontend :
        {
          ...
          "model": "account.move",
          "model_label": "Écritures comptables",
          "method": "write",
          "record_ids": [42],
          "summary": "Modifier 1 « Écritures comptables »",
    ...
    _METHOD_VERBS = {
        "action_post": "Valider / comptabiliser",
        "post": "Valider / comptabiliser",
        "action_confirm": "Confirmer", "button_confirm": "Confirmer",
        "confirm": "Confirmer",
        "action_cancel": "Annuler", "button_cancel": "Annuler",
        "action_draft": "Repasser en brouillon", "button_draft": "Repasser en brouillon",
        "action_validate": "Valider", "button_validate": "Valider",
        "validate": "Valider",
        "action_approve": "Approuver", "approve": "Approuver",
        "action_refuse": "Refuser",
        "action_done": "Terminer",
        "reconcile": "Lettrer / rapprocher",
    }
...
def _whoami(env, params):
    u = env.user
    # The user-group field is `groups_id` on 18 and was renamed to
    # `group_ids` in 19. Probe both so this file stays version-agnostic.
    groups_field = (
...
def _readable_fields(env, model):
    """Les champs de `model` que l'utilisateur COURANT a le droit de lire.

    `env[model]._fields` décrit la DÉFINITION du modèle, sans aucun égard aux
    droits : un champ protégé par `groups=` y figure pour tout le monde. Le
    demander lève alors une AccessError qui casse le tour de chat ENTIER,
    au lieu d'omettre simplement la colonne interdite.

    Le cas qui l'a révélé : `api_key` sur digi_erp.model.llm.provider est
    réservé aux administrateurs DIGI-ERP AI. L'assistant, à qui on annonçait
    ce champ comme disponible, le demandait — et l'utilisateur recevait
    « Vous n'avez pas assez de droits pour accéder aux champs api_key ».
    Ce n'est pas propre à ce module : le cœur d'Odoo restreint quantité de
    champs de la même façon, donc le garde-fou vaut pour TOUS les modèles.

    `Field.is_accessible(env)` est exactement le test que `fields_get`
    applique lui-même (odoo/models.py::fields_get), donc ce que l'assistant
    voit ici coïncide avec le schéma qu'on lui a décrit.
    """
    return {fname for fname, field in env[model]._fields.items()
            if field.is_accessible(env)}
...
def _odoo_search_read(env, params):
    ...
    fields = requested
    if requested:
        defined = set(env[model]._fields.keys())
        valid = _readable_fields(env, model)
        fields = [f for f in requested if (f.split(".")[0] in valid)]
        # On sépare « ce champ n'existe pas » de « vous n'y avez pas droit » :
        # le modèle doit réessayer dans le premier cas, surtout pas dans le
        # second (il boucle sinon sur un champ qu'il n'obtiendra jamais).
        forbidden = [f for f in requested
                     if f not in fields and f.split(".")[0] in defined]
        dropped = [f for f in requested
                   if f not in fields and f not in forbidden]
    ...
    if not fields:
        names = sorted(
            fn for fn in _readable_fields(env, model)
    ...
    if dropped:
        ...
        note = (
            ...
            f"Utilise odoo_fields_get({model}) pour les noms exacts. "
            f"N'invente JAMAIS de valeurs pour ces champs."
        )
        out["note"] = (out.get("note", "") + " " + note).strip()
    if forbidden:
        out["restricted_fields"] = forbidden
        note = (
            f"These fields exist on {model} but THIS USER may not read them: "
            f"{forbidden}. They were left out of the result. Do NOT ask for "
            f"them again and do NOT invent their values — answer with the "
            f"data you have, and say that this detail is restricted if the "
            f"user needs to know why."
        )
        out["note"] = (out.get("note", "") + " " + note).strip()
    return out
...
def _odoo_read(env, params):
    ...
    requested = params.get("fields") or []
    # Même garde-fou que dans _odoo_search_read : ne demander à l'ORM que des
    # champs LISIBLES par l'utilisateur courant. `read([])` lit sinon TOUTES
    # les colonnes, ce qui suffit à faire échouer la lecture d'un modèle dès
    # qu'il porte un seul champ protégé par `groups=`.
    readable = _readable_fields(env, model)
    if requested:
        fields = [f for f in requested if f.split(".")[0] in readable]
        forbidden = [f for f in requested if f not in fields]
    else:
        fields = sorted(
            fn for fn in readable
            if fn not in _NOISE_FIELD_EXACT
            and not any(fn.startswith(p) for p in _NOISE_FIELD_PREFIXES)
        )
        forbidden = []
    ...
    if forbidden:
        out["restricted_fields"] = forbidden
        out["note"] = (
            f"Fields left out because this user may not read them on "
            f"{model}: {forbidden}. Do not ask for them again, and never "
            f"invent their values."
        )
    return out
...
def _odoo_call(env, params):
    ...
    if not model or not method:
        return {"error": "Parameters `model` and `method` are required."}
    if model not in env:
        return {"error": f"Unknown model: {model}"}
    if method.startswith("_"):
        return {"error": f"Private method refused: {method}"}
    if method in DENIED_METHODS:
        return {"error": f"Method blocked by the gateway: {method}"}
    args = params.get("args") or []
    kwargs = params.get("kwargs") or {}
    if not isinstance(args, list) or not isinstance(kwargs, dict):
        return {"error": "`args` must be a list and `kwargs` an object."}

    Model = env[model]
    fn = getattr(Model, method, None)
    if fn is None:
        return {"error": f"Method {model}.{method} does not exist."}

    # ── Liaison des ids : la convention RPC d'Odoo (odoo/api.py::call_kw) ──
    #
    # Une méthode décorée @api.model / @api.model_create_multi s'appelle sur
    # le modèle nu et reçoit `args` tel quel. TOUTE AUTRE méthode est une
    # méthode d'enregistrement : son `self` DOIT être le recordset visé, donc
    # le premier argument positionnel est la liste des ids et n'est PAS
    # transmis à la méthode.
    #
    #   write        args = [[id, …], {champ: valeur}]   ← ids consommés ici
    #   action_post  args = [[id, …]]
    #   create       args = [{champ: valeur}]      (@api.model_create → nu)
    #   search_read  args = [domain, fields]       (@api.model        → nu)
    #
    # Sans cette séparation, `write` arrivait sur BaseModel.write(ids, vals)
    # avec un argument de trop et toute modification confirmée mourait sur
    # « write() takes 2 positional arguments but 3 were given ».
    #
    # C'est la MÊME convention que celle qu'attend `build_action_preview` :
    # la carte de confirmation et l'exécution lisent désormais `args` de la
    # même façon, donc ce qui est approuvé est exactement ce qui est écrit.
    records = Model
    if not getattr(fn, "_api", None):
        if not args:
            return {"error": (
                f"{model}.{method} acts on existing records: pass their ids "
                f"as the first element of `args` "
                f"(e.g. args=[[42], {{...}}] for write, args=[[42]] for "
                f"{method})."
            )}
        raw_ids, args = args[0], args[1:]
        if isinstance(raw_ids, (int, str)):
            raw_ids = [raw_ids]
        if not isinstance(raw_ids, (list, tuple)):
            return {"error": (
                f"The first element of `args` must be the list of record ids "
                f"for {model}.{method}, got {type(raw_ids).__name__}."
            )}
        # Même coercition que l'aperçu : le modèle renvoie parfois les ids en
        # chaînes JSON ("42"). On rejette tout ce qui n'est pas un entier.
        ids = [int(i) for i in raw_ids
               if not isinstance(i, bool) and str(i).lstrip("-").isdigit()]
        if len(ids) != len(raw_ids):
            return {"error": (
                f"Invalid record ids for {model}.{method}: {raw_ids!r}."
            )}
        if not ids:
            return {"error": (
                f"No record targeted: {model}.{method} needs at least one id."
            )}
        records = Model.browse(ids)
        fn = getattr(records, method)

    result = fn(*args, **kwargs)

    # Convertir un recordset en dict compact pour la sérialisation JSON.
...
TOOLS = {
    ...
    "odoo_call": {
        ...
        "description": (
            ...
            "ARGS FOLLOW THE ODOO RPC CONVENTION: to act on EXISTING records, "
            "the FIRST element of `args` is the list of their ids. "
            "Write example: {model:'res.partner', method:'write', "
            "args:[[42], {phone:'+216 71 000 000'}]}. "
            "Button/state example: {model:'account.move', "
            "method:'action_post', args:[[42]]}. "
            "Create takes NO ids (the record does not exist yet): "
            "{model:'account.move', method:'create', "
            "args:[{partner_id:7, move_type:'out_invoice', "
            "invoice_line_ids:[[0,0,{name:'Conseil', price_unit:500}]]}]}. "
            "Neither do the query methods (search, search_read, read_group): "
