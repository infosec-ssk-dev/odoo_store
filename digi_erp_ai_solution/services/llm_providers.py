...
def call_chat(provider, messages, tools=None):
    ...
    try:
        if pt == "openai":
            ...
            return {"ok": False, "error": f"Unsupported provider type: {pt}"}
    except requests.exceptions.Timeout:
        _cprint("red", "LLM TIMEOUT", f"{label}")
        return {"ok": False, "error": f"Timeout: {label} took too long."}
    except requests.exceptions.ConnectionError as e:
        _cprint("red", "LLM CONN ERR", f"{label}: {e}")
        return {"ok": False, "error": f"Cannot reach {label}: {e}"}
    except Exception as e:
        _logger.exception("LLM provider error (%s)", label)
        _cprint("red", "LLM ERR", f"{label}: {type(e).__name__}: {e}")
...
def _call_gemini(provider, messages, tools):
    ...
    url = f"{base}/models/{provider.model_name}:generateContent?key={provider.api_key}"
