...
def _guarded(fn):
    """Never let an unexpected exception reach the browser as a traceback.

    Every jsonrpc route here answers with a plain {"ok": ..., "error": ...}
    dict, and the chat renders `error` as an ordinary red bubble. Without
    this, anything unforeseen \u2014 a stale browser tab pointing at a
    deleted conversation, a record rule denying an id, a bug \u2014 comes
    back as a JSON-RPC fault and Odoo shows the end user a Python traceback
    dialog. That is never the right thing to show someone who only asked a
    question, and the details belong in the log where an administrator can
    read them.

    Odoo errors that already carry a readable, translated sentence
    (UserError, ValidationError) are passed through as that sentence.
    """
    @functools.wraps(fn)
    def wrapper(*args, **kwargs):
        try:
            ...
            _logger.info("DIGI-ERP AI: %s denied or missing record", fn.__name__)
            return {"ok": False, "error": _(
                "That conversation is not available any more. Start a new "
                "one from the left-hand panel.")}
        except Exception:
            _logger.exception("DIGI-ERP AI: unhandled error in %s", fn.__name__)
            return {"ok": False, "error": _(
                "Something went wrong on the server. Please try again \u2014 "
                "if it keeps happening, an administrator will find the "
                "details in the Odoo log.")}
    return wrapper
...
def _soffice_convert(blob, src_name, target_ext):
    ...
    if not soffice:
        return None, _(
            "Reading the old Office formats (.ppt / .doc) is unavailable: "
            "install LibreOffice (soffice) on the server.")
    fmt = target_ext.lstrip('.')          # 'pptx' / 'docx'
    # Fixed stem, user extension. The uploaded name never reaches the argv:
    # soffice reads a leading '-' as an option, and we have no reason to let
    # an uploaded filename decide anything about the command line.
    src_base = "source" + (os.path.splitext(src_name)[1] or "")
    ...
    try:
        ...
        if name.endswith('.7z'):
            try:
                ...
                return [], _("Reading .7z is unavailable "
                             "(install `py7zr`).")
            out = []
        ...
        if name.endswith('.rar'):
            try:
                ...
                return [], _("Reading .rar is unavailable (install "
                             "`rarfile` and the `unrar` utility).")
            out = []
...
def _extract_text_from_file(filename, blob, provider_id=None, want_layout=False,
    ...
    if name.endswith('.pdf'):
        try:
            ...
            return _ret(None, _("Reading PDF is unavailable "
    ...
    if name.endswith('.docx'):
        try:
            ...
            return _ret(None, _("Reading DOCX is unavailable "
                                "(install `python-docx`)."))
        try:
            ...
            return _ret('\n'.join(paragraphs).strip()
                        or _("(Empty document.)"), None)
        except Exception as e:
            _logger.warning("DOCX extraction failed (%s): %s", filename, e)
            return _ret(None, _("Could not read the DOCX: %s", e))
    ...
    if name.endswith(('.xlsx', '.xlsm')):
        try:
            ...
            return _ret(None, _("Reading Excel is unavailable "
                                "(install `openpyxl`)."))
        try:
    ...
    if name.endswith('.xls'):
        try:
            ...
            return _ret(None, _("Reading legacy XLS is unavailable "
                                "(install `xlrd<2`)."))
        try:
    ...
    if name.endswith('.pptx'):
        try:
            ...
            return _ret(None, _("Reading PPTX is unavailable "
                                "(install `python-pptx`)."))
        try:
            ...
            return _ret(None, _("Could not read the PPTX: %s", e))
    ...
    if name.endswith(ARCHIVE_EXTS):
        ...
        for inner_name, inner_blob in members:
            ...
            if inner_text:
                ...
                n_files += 1
        if not chunks:
            return _ret(_("(Empty archive, or no readable file inside.)"), None)
        return _ret("\n".join(chunks), None)
...
def _build_prompt_with_doc(prompt, filename, extracted_text):
    ...
    return (
        "You are a helpful assistant. The user attached the document below. "
    ...
    try:
        ...
        if not resp.ok:
            ...
            if "not found" in low or "no such model" in low:
                _cprint("yellow", "LOCAL /chat NO-MODEL", f"{model}: {err_msg}")
                return {"ok": False, "error": _(
                    "The AI server does not have a model called "
                    "\u201c%s\u201d. Download it on the server, or correct "
                    "the model name under Settings \u2192 DIGI-ERP AI.",
                    model)}
            _logger.warning("Local /api/chat refused the request: %s", err_msg)
            return {"ok": False, "error": _(
                "The AI server refused the request: %s", err_msg)}
        ...
        # Never surface the raw exception: the user cannot act on
        # "InvalidURL: Failed to parse: http://…" and it exposes internals.
        # The full traceback is in the Odoo log for whoever administers this.
        _logger.exception("Local /api/chat: unexpected error")
        _cprint("red", "LOCAL /chat ERR", f"{type(e).__name__}: {e}")
        return {"ok": False, "error": _(
            "The AI server could not be reached. Check that it is running "
            "and that its address is correct under Settings \u2192 "
            "DIGI-ERP AI. The details are in the Odoo log.")}
...
def _first_run_hint(env):
    """Message for a database where no AI has been connected yet, or None.

    Returns None as soon as ANYONE has done something deliberate — added a
    model record, or edited the built-in server settings — because from then
    on a connection error is a real connection error worth reporting.
    """
    if env["digi_erp.model.llm.provider"].sudo().search_count(
            [("active", "=", True)]):
        return None
    params = env["ir.config_parameter"].sudo()
    if params.get_param("digi_erp_ai.local_base_url") or \
       params.get_param("digi_erp_ai.chat_model"):
        return None
    if env.user.has_group("digi_erp_ai_solution.group_ai_manager"):
        return _("No AI is connected yet. Open DIGI-ERP AI \u2192 AI Models "
                 "and add one \u2014 an online service with its API key, or a "
                 "model you run yourself \u2014 and it will appear in the "
                 "picker above.")
    return _("No AI is connected yet. Ask whoever administers your Odoo to "
             "add one under DIGI-ERP AI \u2192 AI Models.")


def _fallback_provider(env):
    """The model to use when nothing was explicitly picked, or None.

    Auto-routing returns nothing when no model has a tier set, and "Use this
    model for" is optional — so an admin who connects one model and leaves
    that field empty would otherwise have every question fall through to the
    built-in localhost server and fail with an address they never entered.

    Returning None keeps the built-in path, which is correct for anyone who
    deliberately configured it; that is why an explicitly-set server URL or
    chat model wins over this fallback.
    """
    params = env["ir.config_parameter"].sudo()
    if params.get_param("digi_erp_ai.local_base_url") or \
       params.get_param("digi_erp_ai.chat_model"):
        return None
    return env["digi_erp.model.llm.provider"].sudo().search(
        [("active", "=", True)], order="id asc", limit=1) or None
...
def _call_chat(env, provider_id, messages, tools=None, conv=None,
    ...
    if provider is None:
        # FIRST RUN. Nobody has registered an AI model, and nobody has
        # touched the built-in server settings either — so the only reason
        # we are about to call http://localhost:11434 is that it is the
        # shipped default. Telling a brand-new user "Cannot reach
        # http://localhost:11434" is nonsense: they never typed that URL and
        # cannot act on it. Say what is actually missing, and address the
        # person who can fix it.
        first_run = _first_run_hint(env)
        if first_run:
            return {"ok": False, "error": first_run,
                    "model_label": _provider_label(env, None)}
        provider = _fallback_provider(env)
...
class DigiErpAiChatController(http.Controller):
    ...
    @http.route('/ai-solution/providers', type='jsonrpc', auth='ai_user',
    ...
    @http.route('/ai-solution/companies', type='jsonrpc', auth='ai_user',
                methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/conversations', type='jsonrpc', auth='ai_user', methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/conversations/new', type='jsonrpc', auth='ai_user', methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/conversations/<int:conversation_id>/rename',
                type='jsonrpc', auth='ai_user', methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/ask', type='jsonrpc', auth='ai_user', methods=['POST'], csrf=False)
    @_guarded
    def ai_ask(self, prompt='', conversation_id=None, use_tools=True,
                  provider_id=None, company_ids=None, **kwargs):
        prompt = (prompt or '').strip()
        if not prompt:
            return {'ok': False, 'error': _("Please type a question first.")}
    ...
    @http.route('/ai-solution/cancel', type='jsonrpc', auth='ai_user',
                methods=['POST'], csrf=False)
    @_guarded
    def ai_cancel(self, conversation_id=None, **kwargs):
        ...
        someone else's turn."""
        if not conversation_id:
            return {"ok": False, "error": _("No conversation to stop.")}
        conv = self._get_conversation(conversation_id)
        if not conv:
            return {"ok": False, "error": _("Conversation not found.")}
    ...
    @http.route('/ai-solution/confirm-action', type='jsonrpc', auth='ai_user',
                methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/ask-context', type='jsonrpc', auth='ai_user',
                methods=['POST'], csrf=False)
    @_guarded
    ...
    @http.route('/ai-solution/embedded/conversations', type='jsonrpc',
    ...
    @http.route('/ai-solution/embedded/conversations/new', type='jsonrpc',
                auth='ai_user', methods=['POST'], csrf=False)
    @_guarded
    ...
    # CSRF stays ON here, unlike the jsonrpc routes above. This is the only
    ...
    def _do_ask_with_file(self, prompt, conversation_id, provider_id,
        ...
        if len(blob) > MAX_FILE_BYTES:
            ...
            return _json({
                'ok': False,
                'error': _("File too large (max %s MB).",
    ...
    def download_generated_file(self, attachment_id, **kwargs):
        ...
        return Response(
            ...
            headers={
                # content_disposition() and not an f-string: the name comes
                # from the model's `filename` parameter, so a quote or a
                # newline in it would break out of the header. Core's helper
                # does the RFC 6266 percent-encoding.
                "Content-Disposition": content_disposition(attach.name or "download"),
